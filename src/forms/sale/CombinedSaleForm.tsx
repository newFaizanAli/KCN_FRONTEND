...
import { SalesOrder, SaleOrderItem } from '../../types'
import { useSaleOrderStore, useCustomerStore, useProductStore } from '../../store'
...
interface CombinedSaleFormValues {
    // Order fields
...
const CombinedSaleForm = () => {
    const { addCombinedSaleOrder } = useSaleOrderStore()
    ...
    const {
        ...
        setValue,
        formState: { errors },
    } = useForm<CombinedSaleFormValues>({
        defaultValues: {
            customerId: null,
            customerName: '',
            status: 'pending',
    ...
    const handleProductChange = async (index: number, opt: { value: string; label: string }) => {
        ...
        try {
            ...
            if (prod) {
                setValue(`items.${index}.price`, prod.price, { shouldDirty: true })
                const qty = Number(watchedItems[index]?.quantity) || 1
                setValue(`items.${index}.total`, prod.price * qty)
            }
        } catch (err) {
    ...
    const onSubmit = async (data: CombinedSaleFormValues) => {
        ...
        try {
            ...
            await addCombinedSaleOrder(payload)
            reset()
        } catch (err) {
            ...
            <div className="p-6 fade-in">
                <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
                    ...
                    <div className="space-y-4">
                        ...
                        <FormSearchSelect<CombinedSaleFormValues>
                            label="Customer"
                            ...
                            value={
                                watch('customerId')
                                    ? {
                                        value: watch('customerId') as string,
                                        label: watch('customerName') || ''
                                    }
                                    : null
                            }
                                    ...
                                    >
                                        ...
                                        <div className="grid grid-cols-2 gap-3">
                                            ...
                                            <FormInput
                                                ...
                                                error={(errors.items?.[index] as any)?.price?.message}
                                            />
                                        </div>
