...
const UserForm = ({ setModal, isEdit, defaultValues, handleClose }: UserFormProps) => {
    ...
    return (
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
            ...
            <FormInput label="Email" placeholder="user@electrocore.io" icon={Mail} type="email" name="email" register={register}
                rules={{ required: "Email required", pattern: { value: /^[^\s@]+@[^\s@]+\.[^\s@]+$/, message: "Invalid email" } }} error={errors.email?.message} />
            <FormSelect label="Role" name="role" register={register} options={[{ value: "admin", label: "Admin (full access)" }, { value: "sub_admin", label: "Sub Admin (restricted)" }]} />
            {
                !isEdit &&
